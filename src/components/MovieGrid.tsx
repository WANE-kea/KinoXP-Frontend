    ...
    <div className="movie-grid">
      {movies.map((movie: Movie) => (
        <div key={movie.id} className="movie-card">
          {movie.posterBase64 ? (
            <img src={movie.posterBase64} alt={movie.title} style={{ width: "50%" }} onClick={() => showMovieInfo(movie.id)} />
          ) : (
            <img src={movie.posterUrl} alt={movie.title} className="move-image" style={{ width: "50%" }} onClick={() => showMovieInfo(movie.id)} />
          )}
          <h2>{movie.title}</h2>
